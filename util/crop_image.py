...
def general_crop_expand(image, tile, expandRate):
    """Crop the image giving a tile.
    Args:
        image: Image to be crop, [h, w, c].
        tile: [p_0, p_1, p_2, p_3] (clockwise).
    ...
    # phase3:crop the box out.
    (newCX, newCY) = (nW // 2, nH // 2)
    expand_dist = bb_height / expandRate
  ...
  #  print([x_min,y_min,x_max,y_max])
   # print([nW,nH])
    return im_rotate[y_min-5:y_max+5, x_min-5:x_max+5, :], horiz

def crop_image(image_path, detection_results, crop_dir, expandRate):
    ...
    for result in detection_results:
        index = index + 1
        crop_name=str(index)+'_'+ str(expandRate) + '.jpg'
        ...
        if(Polygon(tile).area==0):
            continue
        crop,hori=general_crop_expand(np.array(img),tile, expandRate)
